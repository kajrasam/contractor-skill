...
new_js = """
        ...
        function buildRoleResponseSection() {
            ...
            visiblePos.forEach(pos => {
                ...
                if(isAdmin && isEditMode) {
                    ...
                    html += `<p class="text-xs text-slate-500 leading-relaxed">${response}</p>`;
                }
        ...
        function drawEvalRadar() {
            ...
            const cleanLabels = getLabels().map(l => {
                const parts = l.split('. ');
            ...
            evalRadarChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: cleanLabels, 
                    datasets: [
                        { label: 'Target', data: targets, borderColor: '#cbd5e1', backgroundColor: 'transparent', borderDash: [5,5] },
        ...
        async function saveEvaluation() {
            ...
            for (let i = 0; i < competencies.length; i++) {
                emp.actuals[i] = parseInt(document.getElementById(`eval-val-${i}`).value);
                emp.evidences[i] = document.getElementById(`eval-evi-${i}`).value;
            }
        ...
        function renderIndividualDashboards(subIds) {
            ...
            subIds.forEach(id => {
                ...
                const cleanLabels = currentLabels.map(l => {
                    const parts = l.split('. ');
                    return parts.length > 1 ? parts.slice(1).join('. ') : l;
                });
                ...
                const chart = new Chart(ctx, {
                    ...
                    data: {
                        labels: cleanLabels,
                        datasets: [
                            { label: 'Target', data: positionTargets[emp.position], borderColor: '#94a3b8', backgroundColor: 'rgba(148, 163, 184, 0.2)' },
                            { label: 'Actual', data: emp.actuals, borderColor: '#882239', backgroundColor: 'rgba(136, 34, 57, 0.3)' }
                        ]
                    },
                    options: { responsive: true, maintainAspectRatio: false, scales: { r: { min: 0, max: 5, ticks:{display:false} } } }
                });
