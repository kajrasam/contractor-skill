...
new_js = """
        ...
        function buildIDPFilters() {
            ...
            let visibleUsers = [];
            if (currentUser.id === 'Admin') {
                visibleUsers = Object.keys(dbUsers).filter(uid => uid !== 'Admin');
            } else {
            ...
            visibleUsers.forEach(uid => {
                ...
                if (emp) {
                    ...
                    emps.push({ id: uid, name: emp.name, position: emp.position });
                }
            });
        ...
        function renderIDPContent() {
            ...
            if (!selectedIDPEmp) {
                container.innerHTML = `
                    <div class="bg-white p-10 rounded-2xl border border-slate-100 shadow-sm text-center text-slate-500">
                        ...
                        <p>กรุณาเลือกพนักงานเพื่อดูแผนพัฒนา (IDP)</p>
                    </div>`;
                return;
            }
            
            const emp = dbUsers[selectedIDPEmp];
            const targets = positionTargets[emp.position] || [];
            ...
            container.innerHTML = `
                <!-- IDP Header Card -->
                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-6 flex flex-col lg:flex-row">
                    <div class="p-6 md:p-8 lg:w-2/3 border-b lg:border-b-0 lg:border-r border-slate-100">
                        <div class="flex items-start gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <div class="inline-flex items-center bg-slate-100 px-3 py-1 rounded-full text-sm font-medium text-slate-600 mb-2 border border-slate-200">
                                    <i class="fa-solid fa-briefcase text-scg-500 mr-2"></i> ${emp.position}
                                </div>
                                <div class="text-sm text-slate-500">
                                    <i class="fa-regular fa-calendar-check mr-1"></i> วันที่ประเมินล่าสุด: <span class="font-bold text-slate-700">${emp.evalDate || '-'}</span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                            <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2"><i class="fa-solid fa-clipboard-list text-scg-400 mr-1"></i> บทบาทหน้าที่ความรับผิดชอบ (Role & Responsibilities)</h4>
                            <p class="text-sm text-slate-700 whitespace-pre-wrap">${roleResponses[emp.position] || '-'}</p>
                        </div>
                    </div>
                    
                    <div class="p-6 md:p-8 lg:w-1/3 flex flex-col items-center justify-center bg-slate-50/50">
                        <div class="text-center mb-4">
                            <h4 class="text-sm font-bold text-slate-500 mb-2">% Completed Skill Level</h4>
                            <div class="text-5xl font-black text-scg-700 mb-1">${pctComplete}%</div>
                        </div>
        ...
        function drawIDPRadar(emp, targets) {
            ...
            idpRadarChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: cleanLabels, 
                    datasets: [
                        { label: 'Target', data: cleanTargets, borderColor: '#cbd5e1', backgroundColor: 'transparent', borderDash: [5,5], pointRadius: 0 },
                        { label: 'Actual', data: actuals, borderColor: '#ca3656', backgroundColor: 'rgba(202, 54, 86, 0.2)', pointRadius: 3 }
                    ]
                },
                options: { 
                    responsive: true, 
